...
def _get_user_token():
    """Extract OBO token from Databricks Apps request headers."""
    try:
        return flask_request.headers.get("X-Forwarded-Access-Token")
    except RuntimeError:
        return None
...
def call_llm_for_insights(df, prompt=None, user_token=None):
    """
